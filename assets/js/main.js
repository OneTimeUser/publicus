 ...
 */
{
    ...
    class Cursor {
        ...
        render() {
            this.renderedStyles['tx'].current = mousepos.x - this.bounds.width / 4;
            this.renderedStyles['ty'].current = mousepos.y - this.bounds.height / 4;
    ...
    class MenuItem {
        constructor(el, imageGrid) {
            ...
            this.DOM.content =
                this.DOM.el.querySelector('.menu__item-content');
        ...
        toggleExplorer(action = 'show') {
            ...
            return new Promise((resolve, reject) => {
                TweenMax.to(this.DOM.explore, ANIMATION_SETTINGS.explore.duration, {
                    ...
                    startAt: action === 'hide' ? null : {
                        scale: 0.5
                    },
                    opacity: action === 'hide' ? 0 : 1,
                    scale: action === 'hide' ? .8 : 1,
                    onComplete: resolve
                });
            });
        ...
        // Show/Hide the text 
        showText() {
            return this.toggleText('show');
        }
        hideText() {
            return this.toggleText('hide');
        }
        toggleText(action = 'show') {

            return new Promise((resolve, reject) => {
                TweenMax.to(this.DOM.text, ANIMATION_SETTINGS.text.duration, {
                    ease: ANIMATION_SETTINGS.text.ease,
                    startAt: action === 'hide' ? null : {
                        scale: 0.5
                    },
                    opacity: action === 'hide' ? 0 : 1,
    ...
    class Menu {
        constructor(el) {
            ...
            this.menuItems[this.current].setCurrent().showText();
            ...
            this.renderedStyles = {
                menuTranslation: {
                    previous: this.dragPosition + this.initTx,
                    current: this.dragPosition + this.initTx,
                    amt: 0.1
                },
                letterTranslation: {
                    previous: 0,
        ...
        render() {
            ...
            // Translate the menu
            TweenMax.set(this.DOM.menu, {
                x: this.renderedStyles.menuTranslation.previous
            });
            ...
            if (this.isDragging() && this.currentItem && this.upcomingItem) {
                ...
                TweenMax.set(this.currentItem.letters, {
                    x: this.dragDirection === 'left' ? -1 * tx - 100 + '%' : tx - 100 + '%'
                });
                TweenMax.set(this.upcomingItem.letters, {
                    x: this.dragDirection === 'left' ? tx + '%' : -1 * tx + '%'
                });
                ...
                TweenMax.set(this.upcomingItem.imageGrid.DOM.images, {
                    ...
                    x: this.dragDirection === 'left' ? 150 - this.renderedStyles.imgTranslation.previous + '0%' : -1 * (150 - this.renderedStyles.imgTranslation.previous) + '%'
                });
            }
        ...
        initEvents() {
            ...
            this.onDragMove = (event, pointer, moveVector) => {
                ...
                if (this.dragDirection === '') {
                    ...
                    this.menuItems[this.current].hideText();
            ...
            this.onDragEnd = () => {
                if (!this.isAnimating) {
                    ...
                    if (Math.abs(this.dragPosition) <= this.minDrag || this.dragDirection !== this.currentDirection) {
                        ...
                        this.menuItems[this.current].showText();
                    ...
                    // Move to the next/previous menu item
                        ...
                        this.menuItems[this.upcomingIdx].showText();



                        // Set the updated menu translation value
                        this.currentPosition = this.renderedStyles.menuTranslation.current = this.renderedStyles.menuTranslation.previous = this.dragDirection === 'left' ?
        ...
        showContent() {
            ...
            promises.push(this.menuItems[this.current].hideText());
        ...
        hideContent() {
            ...
            scrollIt(0, 300, 'easeOutQuad', () => {
                ...
                promises.push(this.menuItems[this.current].showText());
    ...
    /***********************************/
    /****** Menu related ******/
    //    var cats = document.querySelectorAll('.menu__menu-cat > .titles');
    //
    //    console.log(cats);

    //    cats.forEach(link => {
    //        link.addEventListener('mousenter', event => {
    //            console.log("entered inside");
    //        });
    //    });

    function menuSwitcher(cat) {
        //grab data attribute for string for getting ID var
        var menuItems = cat.getAttribute('data-attribute');

        //get current active category to be changed
        var oldCat = document.querySelector('.active-menu-cat');

        var oldAttr = oldCat.getAttribute('data-attribute');

        //get current menu items to be changed
        var oldItems = document.getElementById(oldAttr);


        //remove active class from old
        document.querySelector('.active-menu-cat').classList.remove('active-menu-cat');

        //change display of children to none
        var oldChildren = oldItems.children;
        for (var i = 0; i < oldChildren.length; i++) {
            oldChildren[i].style.display = "none";
        }

        //add active class to new 
        cat.classList.add("active-menu-cat");

        //get new menu items 
        var newItems = document.getElementById(menuItems);

        //change display to block
        var newChildren = newItems.children;
    ...
    Promise.all([
        ...
        //
        //        const menuCat = parentDOM.getElementsByClassName("titles");
        //
        //        function menuFunction() {
        //            console.log("is this auto?");
        //
        //        }
        //
        //        for (var i = 0; i < menuCat.length; i++) {
        //            menuCat[i].addEventListener('click', menuFunction(), false);
        //        }
